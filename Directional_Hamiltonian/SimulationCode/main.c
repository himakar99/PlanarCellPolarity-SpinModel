...
int main(){
    ...
    // Interaction parameters. m1 positive, m2 negative. Global alignment field parameter m3, negative.
    double m1,m2;
    double m3;
    m1 = 1;
    m2 = -1;
    ...
    //0 = all cells identically completly algined in any of the six directions; 1 = random; 2 = all cells completely aligned in 0-direction
    int randSpin = 2;

    //Total number of mcmc-steps; change numerical value ONLY
    unsigned long long mcmcSteps = (dim*dim)*(long long)1000000;

    // Number of initial MC steps to be discarded; change numerical value ONLY
    unsigned long long burningPhase = (dim*dim)*(long long)0; 
    ...
    double energy;
    energy = energy_calc(nbr, spin, totalcell, m1, m2, m3);
    fprintf(fp1, "%lf\n", (energy/totalcell));
    ...
    for(i = 1; i<=burningPhase; i++){
        energy = energy + mcmc(spin, m1, m2, m3, dim, totalcell, nbr, r);
    }
