...
double mcmc(int **spin, double m1, double m2, double m3, int dim, int totalcell, int **nbr, gsl_rng * r){
    ...
    if (selectSpin1 == selectSpin2){
        ...
        int h3_old = 0;
        h3_old = ScellSpin[0]+ScellSpin[5];
        ...
        int h3_new = 0;
        h3_new = ScellSpin[0]+ScellSpin[5];
        
        del_h2 = m2*(h2_new - h2_old);
        del_h3 = m3*(h3_new - h3_old);
